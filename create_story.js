...
async function handleDocument(
  ...
  const publishedRevisionId = await getPublishedRevisionId(
    fusionbase,
    fusiontoken,
    documentId
  );
  const existingContent = await getRevisionContent(
    fusionbase,
    fusiontoken,
    documentId,
    publishedRevisionId
  );
  const revisedContent = reviseContent(
    existingContent,
    targetAuthor,
    replacementAuthorId
  );
...
async function publishStory(FUSION_BASE, FUSION_TOKEN, storyContent) {
  ...
  await circulateStory(
    ...
    documentId,
    new Date(),
    slugify(storyContent.headlines.basic)
  );
  const results = await publishDraftRevision(
    FUSION_BASE,
    FUSION_TOKEN,
    documentId
  );
...
async function main(FUSION_BASE, FUSION_TOKEN, storyContent) {
  //   const storyContent = {
